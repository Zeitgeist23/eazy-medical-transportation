(()=>{
  ...
  async function loadIndex(){if(!indexPromise)indexPromise=fetch('/data/provider-name-index.json?v=3',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('Provider name index unavailable');return r.json()});return indexPromise}
  async function loadProfiles(){if(!profilePromise)profilePromise=fetch('/providers/index.json?v=3',{cache:'no-store'}).then(r=>r.ok?r.json():[]).then(a=>Object.fromEntries(a.map(x=>[String(x.npi),x.url]))).catch(()=>({}));return profilePromise}
  function resultsHtml(list,q,profiles){if(!list.length)return `<div class="claim-callout">No provider-name matches were found for “${esc(q)}”.</div>`;return `<div class="claim-callout">Provider-name matches from the national Eazy NEMT directory.</div><div class="results">${list.map(p=>{const u=profiles[String(p.npi)]||'';return `<div class="result"><h3>${esc(p.name)}</h3><p>${esc(p.address1||'')}${p.address1?'<br>':''}${esc(p.city||'')}, ${esc(p.state||'')} ${esc(String(p.zip||'').slice(0,5))}${p.phone?`<br>${esc(p.phone)}`:''}${p.npi?`<br>NPI: ${esc(p.npi)}`:''}</p><div class="tags" style="margin-top:8px"><span class="tag">NEMT</span></div><div class="result-actions">${u?`<a class="btn-primary" href="${esc(u)}">Provider Profile</a>`:''}${p.phone?`<a class="btn-secondary" href="tel:${esc(p.phone)}">Call Provider</a>`:''}</div></div>`}).join('')}</div>`}
